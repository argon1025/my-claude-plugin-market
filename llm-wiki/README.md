...
- **확인 필요 인박스**: 무인 갱신이 판정하지 못한 충돌과 근거를 얻지 못한 교체 요청은 도메인별 `inbox/{domain}.md`에 쌓이고 현재 레포 도메인의 건수만 세션 헤더에 표시되며 `/llm-wiki:add`가 소비함 — 담당자가 다른 도메인이 한 파일을 append하며 생기는 push 충돌을 피하기 위해 나눔
