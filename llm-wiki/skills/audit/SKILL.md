...
- **레포**: 대상 도메인 레포마다 `--repo {slug}`를 붙인 `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/update.py" mirror --wiki {WIKI_ROOT}` 출력을 같은 `catalog.md`에 이어 저장
...
기준: `sed -n '/^## 1\./,/^## 8\./p' {doc_contract_path}` — 1~7장, 1장 판정의 코드 확인은 목록의 레포 줄로 `git -C {경로} grep {패턴} {브랜치}`·`git -C {경로} show {브랜치}:{파일}`을 실행해 수행, `없음` 레포의 불릿은 판정 생략.
