...
- **새로 만들 때**: `knowledge/common/`·`state/` 생성, `.gitignore`에 `.local/`, `registry.json`에 `{"projects": {}, "repos": {}}`를 쓰고 첫 커밋 — 원격 URL을 받았으면 `git remote add origin`과 `push -u`
- **원격 없음 경고**: 원격을 끝내 못 받으면 "다른 머신·팀원과 공유되지 않음"을 보고에 남김

## 2. 레포 등록

- **대상**: 현재 디렉터리가 git 레포이고 `registry.json`에 없을 때만 — 이미 등록됐으면 경로만 갱신하고 3장으로
- **slug**: origin(없으면 upstream) URL의 마지막 경로 요소에서 `.git` 제거·소문자·비허용 문자 하이픈 치환, remote가 없으면 `git rev-parse --path-format=absolute --git-common-dir`의 부모 디렉터리명 — 이미 쓰는 slug와 겹치면 `{project}-{name}`을 제안
- **질문 한 라운드**: AskUserQuestion으로 프로젝트(기존 목록 또는 신규)·기본 브랜치·한 줄 summary를 함께 물음 — stack은 `package.json`·`pom.xml`·`build.gradle`에서 읽어 초안으로 제시
- **기록**: `registry.json`의 `repos.{slug}`에 `project`·`remotes`·`branch`·`stack`·`summary`, 신규 프로젝트면 `projects.{p}.summary`도 함께
...
- **목록**: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/catalog.py" --root {WIKI_ROOT}/knowledge/common --label 공통` 출력
