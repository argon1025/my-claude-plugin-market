...
- **검사·커밋**: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/catalog.py" --check --root {WIKI_ROOT}/knowledge {바꾼 파일}` 에러 0 확인 후 문서마다 커밋하되 기존 값을 덮은 커밋은 규약 8장 교체 사유 한 줄을 본문에 담고, `inbox/{domain}.md`의 처리한 행 삭제와 4장이 새로 기재한 행을 같은 실행의 마지막 커밋에 담은 뒤 `git pull --rebase && git push`
