...
- **실행**: `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/update.py" pending --wiki {WIKI_ROOT} --out {스크래치} {인자}` — 레포별 커서 이후 first-parent 커밋 목록과 머지별 diff 파일을 만들고 `work.json` 경로를 마지막 줄에 냄
...
한 메시지에 병렬 Agent 호출, `model: sonnet`, 출력은 파일 Write, 응답은 건수만 — 사실이 메인 컨텍스트를 지나지 않아야 한 세션이 수십 건을 처리합니다. 머지 3건 이하면 메인이 직접 처리하고, 응답 없는 머지는 3장 전에 재실행합니다.
...
한 메시지에 병렬 Agent 호출, `model: sonnet`. 같은 파일을 두 에이전트가 고치지 않게 문서 단위로 묶고, 에이전트는 커밋하지 않습니다.
