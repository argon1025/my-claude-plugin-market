...
- **건너뜀**: `work.json`의 `skipped`(로컬 경로 없음·force-push 의심·대상 ref 없음·제외 레포)와 `notes`는 그대로 보고에 옮김 — `제외 레포`는 `status`가 `excluded`인 노드라 되살리려면 `/llm-wiki:register`로 status를 바꿔야 함
...
코드 검색 한 번으로 확인되는 문장(시그니처·타입·enum·파일 위치·한 레포 안 호출 관계·한 심볼의 내부 동작)과
이번 변경의 서사("A를 B로 바꿈")는 사실이 아닙니다. 남는 것은 코드에 없는 결정·제약·금지·외부 계약·컨벤션입니다.

레포 간 호출·소비 관계는 사실이 아니라 deps입니다 — facts에 넣지 말고 deps에 넣으세요.
방향은 "이 레포가 상대의 계약에 의존"이며, 빌드 의존은 `library`, HTTP 클라이언트·Feign은 `http`,
큐·토픽 리스너는 `message`(이 레포가 소비하므로 target은 발행 측 식별자), 타 레포 스키마 직접 조회는 `data`입니다.
areas의 area는 아래 기존 키 중 하나만 쓰고 걸리는 것이 없으면 새 이름을 적으세요 — {그 레포 도메인의 기존 영역 키 목록}.

출력: {facts_dir}/{slug}/{batch_id}.json에 Write — 아래 4키 객체 1개.
{"facts": [{"fact": "현재 상태를 말하는 한 문장(업무 낱말 우선, 식별자는 괄호 병기, 줄바꿈 금지)",
            "topic": "2~4낱말 주제",
            "code": "저장소 상대 경로 또는 파일#심볼 — 필수",
            "quote": "커밋 메시지·노트의 결정 근거 원문 — 있을 때만",
            "shas": ["근거 머지 sha7 — 1개 이상"]}],
 "deps": [{"target": "호스트명·group:artifact·큐 이름·레포 이름", "kind": "library|http|message|data",
           "identifiers": ["계약 식별자"], "shas": ["sha7"]}],
 "areas": [{"area": "위 목록의 기존 키 또는 새 이름", "role": "40자 이내 역할", "shas": ["sha7"]}],
 "hosts": [{"host": "이 레포가 서빙·발행하는 식별자", "shas": ["sha7"]}]}
빈 항목은 []로 두세요. 응답은 키별 건수만 반환하세요.
````
...
- **간선 배정**: `deps[].target`을 `registry.json`의 다른 노드 `hosts`·slug·`remotes`와 대조 — 걸리면 `(from = 현재 레포, to = 대상, kind)` 셋으로 `관찰` 간선을 더하거나, 같은 셋의 기존 간선 `note`에 `identifiers`를 보태고 `source`를 갱신함(`관찰` 간선만 갱신, `확인` 간선의 `note`는 식별자 추가만). 걸리지 않으면 `기각 — 미등록 상대`로 두고 대상 원문을 보고에 남김
- **영역 배정**: `areas[].area`가 프롬프트에 전달한 기존 키면 현재 레포 노드의 `areas`에 `{domain}/{영역}: {역할}`로 보강(이미 있으면 건너뜀), 전달 키 밖 이름이면 노드를 고치지 않고 `inbox/{domain}.md`에 `- [{날짜}] [{domain}] 새 영역 후보 — 기존 없음 / 새 {제안 키}: {역할} ({slug} @{sha7} {code})` 행 — 어휘 확장은 `확인` 등급인 register·add만 함
- **호스트 배정**: `hosts[].host`가 다른 노드의 `hosts`와 겹치지 않을 때만 현재 레포 노드에 추가 — 겹치면 `기각 — 호스트 중복`
- **배정표**: `{스크래치}/assign.json`에 문서별 사실 목록·신규 여부와 `graph`(간선·영역·호스트 배정)를 씀 — `--dry-run`이면 배정표를 보고하고 종료

## 4. 반영 fan-out — 문서 1장 = 에이전트 1회

한 메시지에 병렬 Agent 호출, `model: sonnet`. 같은 파일을 두 에이전트가 고치지 않게 문서 단위로 묶고, 에이전트는 커밋하지 않습니다. `registry.json`·`deps.json`은 문서가 아니므로 에이전트에 보내지 않고 메인이 규약 9·10장의 키·유일성·정렬 규칙대로 직접 편집합니다.
...
- **그래프**: 간선·영역·호스트 추가 건수와 `미등록 상대` 목록 — 그 레포를 등록하거나 상대 노드의 `hosts`를 `/llm-wiki:add`로 보강하면 다음 실행부터 이어짐
