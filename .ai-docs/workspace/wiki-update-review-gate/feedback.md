...
- `context` audit 진단은 규약 1장 판정에 한해 레포 읽기를 허용하고, 대상은 `.local/paths.json` 로컬 경로의 `defaultBranch` ref이며 로컬 경로가 없는 레포의 불릿은 판정을 생략함 — 로컬 브랜치가 원격보다 뒤처지면 과거 코드 기준으로 판정되는 한계가 있음
  - source: 사용자 확인 2026-09-25
- `correction` 규약 1장 판정을 "코드에 보이면 삭제"로 읽으면 외부 연동 폴더·레이어·인터페이스 배치 같은 구성 컨벤션까지 지워지는데, 코드는 한 사례만 보여 주고 이후 같은 종류 작업이 따라야 할 규칙인지는 드러내지 못하므로 일반 규칙 문장으로 담음 — 사례 서술(특정 폴더명·현재 구현 범위)만 삭제 대상
  - source: 사용자 지적 2026-09-25
