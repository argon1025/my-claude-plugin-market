...
- `correction` planning 스킬 7절의 "정지" 불릿만으로는 승인 후 구현 진입을 막지 못한다 — PR #8에서 계획 스냅샷 커밋 4525329 뒤 57초 만에 같은 세션의 구현 커밋 275c355가 이어졐고, 원인은 ExitPlanMode 승인 결과에 하네스가 "구현을 시작하라"는 지시를 함께 넣기 때문이다.
